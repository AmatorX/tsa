...
def create_monthly_sheets(service, spreadsheet_url, materials):
    ...
    all_month_names = ["January", "February", "March", "April", "May", "June",
                       "July", "August", "September", "October", "November", "December"]
    current_month_name = all_month_names[current_month - 1]
    ...
    # Создаем лист для фотографий, если он еще не существует
    photos_sheet_title = f"photos_{current_month_name}_{year}"
    if photos_sheet_title not in existing_sheets:
        photos_request = {
            'addSheet': {
                'properties': {
    ...
    # Создаем лист для результатов, если он еще не существует
    results_sheet_title = f"results_{current_month_name}_{year}"
    if results_sheet_title not in existing_sheets:
        results_request = {
            'addSheet': {
                'properties': {
    ...
    if photos_sheet_title not in existing_sheets:
        create_photos_table(service, spreadsheet_url, current_month_name)

    if results_sheet_title not in existing_sheets:
        create_materials_table(service, spreadsheet_url, current_month_name, materials)
    print(f"Monthly sheets for {current_month_name} {year} checked/created in spreadsheet: {spreadsheet_url}")
    return f"Monthly sheets for {current_month_name} {year} checked/created in spreadsheet: {spreadsheet_url}"
